...
export default function TestPromesas() {
  ...
  useEffect(() => {
    //Uso el useEffect xq estoy seguro que se va a ejecutar una sola vez (la primera)

    setLoading(true);
    setError(false);
    setResultado([]);
    const pagara = new Promise((resolve, reject) => {
      //Esta es la forma de hacer una promesa
      setTimeout(() => {
    ...
    pagara
      ...
      .catch((error) => {
        //Cuando se resuelva, fijate que resultado hay y mostra
        setError(true);
        setLoading(false);
        console.log(error);
